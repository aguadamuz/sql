...
, signers as --our cohort is made up of first time signers with first sign on event 4623766 who also received an action alert for event_id: 4803238
(
with  first_sig as (
	select * from(
...
---------------------------------------------
---------------------------------------------
--users who didn't receive the AA

WITH email_recipients AS
(
--our cohort should consist of users that were NOT sent an action alert for event_id: 4803238
SELECT DISTINCT
aaue.user_id
FROM action_alert_user_events aaue
JOIN action_alert_records aar ON aaue.action_alert_record_id = aar.id
